...
def test_draw_circles_from_rectangle():
    ...
    ####################################################################
    # ------------------------------------------------------------------
    title = 'Tests 1 and 2 of DRAW_CIRCLES_FROM_RECTANGLE: 8 blue in row, 3 in column; then 4 green in row, 5 in column.'
    window1 = rg.RoseWindow(720, 500, title)


    rect1 = rg.Rectangle(rg.Point(600, 400), rg.Point(500, 450))
    rect1.outline_color = 'red'
    rect1.fill_color = 'blue'
    rect1.outline_thickness = 3
    draw_circles_from_rectangle(8, 3, rect1, window1)

    rect2 = rg.Rectangle(rg.Point(400,250), rg.Point(440,325))
    rect2.outline_color = 'black'
    rect2.fill_color = 'green'
    rect2.outline_thickness = 5
    draw_circles_from_rectangle(4, 5, rect2, window1)
    window1.close_on_mouse_click()

    title = 'Test 3 of DRAW_CIRCLES_FROM_RECTANGLE: 6 yellow-filled in row, 10 brown-outlined in column'
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    for i in range(m):
        center = rg.Point(c.x - r2-r1-(2*r1*i),c.y)
        circle = rg.Circle(center, r1)
        circle.fill_color = rectangle.fill_color
        circle.attach_to(window)
    for k in range(n):
        center = rg.Point(c.x ,c.y - r2-r1-(2*r2*k))
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    rc1 = rectangle1.get_center()
    rc2 = rectangle2.get_center()
    r1pt = rectangle1.get_lower_left_corner()
    dx = r1pt.x - rc1.x
    dy = r1pt.y - rc1.y
    for i in range(n):
        srtPt = rg.Point(rc1.x + (dx*i), rc1.y + (dy*i))
        endPt = rg.Point(rc2.x + (dx*i), rc2.y + (dy*i))
        line = rg.Line(srtPt, endPt)
        if (i%2 == 0):
